import { useState } from "react";
function Input_Validation() {
  const[num ,setNum] =useState(" ");

  function handleInput(e) {
 setNum(e.target.value);
}
const [numtwo, setNumtwo] = useState(" ");
function string_Check(){
  {num.startsWith("py") || num.startsWith("Py")? setNumtwo(num):setNumtwo("py" +num) ||setNumtwo("PY"+num) };
}
 
  return (
    <>
      {/*<!-----------------------------Question no#01  ----------------->*/}
      <hr />
      <div className="parent1">
        <h1 style={{color :"Blue", marginLeft: "10vw"}}>Question No#01</h1>
        <input type="text" placeholder="Enter your string here" onChange={handleInput} />
        <br />
        <button onClick={string_Check}>Click to check</button>
        <p>{numtwo}</p>
      </div>
      <hr />
      {/*<!-----------------------------Question no#02  ----------------->*/}

      {/* <div className="parent2">
        <h1 className="main-heading">Question No#02</h1>
        <input type="text" placeholder="Enter your string here" id="remove" />
        <br />
        <input type="text" placeholder="Enter the index" id="position" />
        <br />
        <button onclick="remove_letter()">Click to remove</button>
        <p id="result"></p>
      </div>
      <hr /> */}
      {/*<!-----------------------------Question no#03  ----------------->*/}

      <div className="parent3">
        <h1 className="main-heading">Question No#03</h1>
        <input type="text" placeholder="Enter your string here" id="field1" />
        <br />
        <button onclick="add_New()">Click to check</button>
        <p id="para2"></p>
      </div>
      <hr />
      {/*<!-----------------------------Question no#04  ----------------->*/}
      <div className="parent4">
        <h1 className="main-heading">Question No#04</h1>
        <input type="text" placeholder="Enter your string here" id="field2" />
        <br />
        <button onclick="add_FrontEnd()">Click to check</button>
        <p id="para3"></p>
      </div>
      <hr />
      {/*<!-----------------------------Question no#05  ----------------->*/}

      <div className="parent5">
        <h1 className="main-heading">Question No#05</h1>
        <input
          type="text"
          placeholder="Enter a number to be check"
          id="field3"
        />
        <br />
        <button onclick="check_Multiple()">Click to check Multiple</button>
        <p id="para4"></p>
      </div>
      <hr />
      {/*<!-----------------------------Question no#06  ----------------->*/}

      <div className="parent6">
        <h1 className="main-heading">Question No#06</h1>
        <input
          type="text"
          placeholder="Enter a number to be check"
          id="field4"
        />
        <br />
        <button onclick="add_New3char()">Click to check</button>
        <p id="para5"></p>
      </div>
      <hr />
      {/*<!-----------------------------Question no#07  ----------------->*/}
      <div className="parent7">
        <h1 className="main-heading">Question No#07</h1>
        <input type="number" placeholder="Enter first number" id="num1" />
        <br />
        <input type="number" placeholder="Enter second number" id="num2" />
        <br />
        <input type="number" placeholder="Enter third number" id="num3" />
        <br />
        <button onclick=" largest_Number()">Click to check</button>
        <p id="result1"></p>
      </div>
      <hr />
      {/*<!-----------------------------Question no#08  ----------------->*/}

      <div className="parent8">
        <h1 className="main-heading">Question No#8</h1>
        <input type="number" placeholder="Enter your grade" id="grade" />
        <br />
        <input type="text" placeholder="Enter Exam type" id="exam" />
        <br />
        <button onclick=" grade_Check()">Click to check</button>
        <p id="result2"></p>
        <p id="result3"></p>
      </div>
      <hr />
      {/*<!-----------------------------Question no#09  ----------------->*/}
      <hr />
      <div className="parent1">
        <h1 className="main-heading">Question No#09</h1>
        <input type="text" placeholder="Enter your string here" id="field" />
        <br />
        <button onclick="string_check()">Click to check</button>
        <p id="para1"></p>
      </div>
      <hr />
      {/*<!-----------------------------Question no#10  ----------------->*/}

      <div className="parent2">
        <h1 className="main-heading">Question No#10</h1>
        <input type="text" placeholder="Enter your string here" id="remove" />
        <br />
        <input type="text" placeholder="Enter the index" id="position" />
        <br />
        <button onclick="remove_letter()">Click to remove</button>
        <p id="result"></p>
      </div>
      <hr />
      {/*<!-----------------------------Question no#11  ----------------->*/}

      <div className="parent3">
        <h1 className="main-heading">Question No#11</h1>
        <input type="text" placeholder="Enter your string here" id="field1" />
        <br />
        <button onclick="add_New()">Click to check</button>
        <p id="para2"></p>
      </div>
      <hr />
      {/*<!-----------------------------Question no#12  ----------------->*/}
      <div className="parent4">
        <h1 className="main-heading">Question No#12</h1>
        <input type="text" placeholder="Enter your string here" id="field2" />
        <br />
        <button onclick="add_FrontEnd()">Click to check</button>
        <p id="para3"></p>
      </div>
      <hr />
      {/*<!-----------------------------Question no#13  ----------------->*/}

      <div className="parent5">
        <h1 className="main-heading">Question No#13</h1>
        <input
          type="text"
          placeholder="Enter a number to be check"
          id="field3"
        />
        <br />
        <button onclick="check_Multiple()">Click to check Multiple</button>
        <p id="para4"></p>
      </div>
      <hr />
      {/*<!-----------------------------Question no#014  ----------------->*/}

      <div className="parent6">
        <h1 className="main-heading">Question No#14</h1>
        <input
          type="text"
          placeholder="Enter a number to be check"
          id="field4"
        />
        <br />
        <button onclick="add_New3char()">Click to check</button>
        <p id="para5"></p>
      </div>
      <hr />
      {/*<!-----------------------------Question no#15  ----------------->*/}
      <div className="parent7">
        <h1 className="main-heading">Question No#15</h1>
        <input type="number" placeholder="Enter first number" id="num1" />
        <br />
        <input type="number" placeholder="Enter second number" id="num2" />
        <br />
        <input type="number" placeholder="Enter third number" id="num3" />
        <br />
        <button onclick=" largest_Number()">Click to check</button>
        <p id="result1"></p>
      </div>
      <hr />
      {/*<!-----------------------------Question no#16  ----------------->*/}

      <div className="parent8">
        <h1 className="main-heading">Question No#16</h1>
        <input type="number" placeholder="Enter your grade" id="grade" />
        <br />
        <input type="text" placeholder="Enter Exam type" id="exam" />
        <br />
        <button onclick=" grade_Check()">Click to check</button>
        <p id="result2"></p>
        <p id="result3"></p>
      </div>
      <hr />
    </>
  );
}

export default Input_Validation;
